...
import PerformanceChart from "../components/PerformanceChart.component";
import RecentInvocations from "../components/RecentInvocations.component";
...
export default function App() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const perfRes = await fetch(`${API_CONFIG.BACKEND_URL}${API_CONFIG.ENDPOINTS.PERFORMANCE}`);
        const perfData = await perfRes.json();
        setPerformanceData(perfData.data);
        setLastUpdated(perfData.lastUpdated);

        const invocRes = await fetch(`${API_CONFIG.BACKEND_URL}${API_CONFIG.ENDPOINTS.INVOCATIONS}?limit=30`);
